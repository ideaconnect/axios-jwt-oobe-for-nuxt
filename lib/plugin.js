...
import AxiosJwtOobe from '@idct/axios-jwt-oobe/lib/axios-jwt-oobe'
...
const axiosExtra = {
  ...
  onLogout(fn) {
    this.jwtHandler.setLogoutAction(fn)
  },
  create(options) {
...
const createAxiosInstance = axiosOptions => {
  ...
  axiosBase.CancelToken = Axios.CancelToken
  axiosBase.isCancel = Axios.isCancel
  axiosOptions.instance = axiosBase
  const axios = new AxiosJwtOobe(axiosOptions)
...
export default (ctx, inject) => {
  ...
  const tokenFieldName = '<%= options.tokenFieldName %>'
  const refreshTokenFieldName = '<%= options.refreshTokenFieldName %>'
  const refreshTokenRetrievalUrl = '<%= options.refreshTokenRetrievalUrl %>'
  const loginActionInfo = <%= JSON.stringify(options.loginActionInfo, null, 4) %>
